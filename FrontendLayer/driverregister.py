...
class DriverRegister(tk.Tk):
    ...
    def __init__(self):
        ...
        def license_click(event):

            self.license_in.config(state="normal")
            if self.license_in.get() == "Enter your license plate number: ":
                self.license_in.delete(0, tk.END)
            else:
                pass

        def carmodel_click(event):

            self.carmodel_in.config(state="normal")
            if self.carmodel_in.get() == "Enter your car model: ":
                self.carmodel_in.delete(0, tk.END)
            else:
                pass
        ...
        self.name_in.pack(padx=35, pady=10, ipady=2)
        ...
        self.num_in.pack(padx=35, pady=10, ipady=2)
        ...
        self.address_in.pack(padx=35, pady=10, ipady=2)
        ...
        self.email_in.pack(padx=35, pady=10, ipady=2)
        ...
        self.pass_in.pack(padx=35, pady=10, ipady=2)

        self.pass_in.bind("<Button-1>", pass_click)

        self.license_in = tk.Entry(self.m_panel, width=60, )
        self.license_in.insert(0, "Enter your license plate number: ")
        self.license_in.config(state="disabled")
        self.license_in.pack(padx=35, pady=10, ipady=2)

        self.license_in.bind("<Button-1>", license_click)

        self.carmodel_in = tk.Entry(self.m_panel, width=60, )
        self.carmodel_in.insert(0, "Enter your car model: ")
        self.carmodel_in.config(state="disabled")
        self.carmodel_in.pack(padx=35, pady=10, ipady=2)

        self.carmodel_in.bind("<Button-1>", carmodel_click)
    ...
    def register(self):
        ...
        license = self.license_in.get()
        carmodel = self.carmodel_in.get()

        cus = DriverModel(name, address, email, number, password, license, carmodel)
        b_driver = BDriver(cus)
        try:
            b_cus = b_driver.register_driver()
            if b_cus:
                self.destroy()
                self.openlogin()
